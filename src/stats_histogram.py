...
def display_histogram(device, histogramData, histogramTime):
    ...
    try:
        with open("/sys/class/thermal/thermal_zone0/temp", "r") as temp:
            ...
            height = (
                ((100 - tmpPercent) * (maxBarHeight - minBarHeight)) / 100
            ) + minBarHeight
    except:
        tmpCel = 0
        height = 0
    ...
    maxHistLenght = 105
    histogramHeight = (
        ((100 - cpuPercent) * (minHistHeight - maxHistHeight)) / 100
    ) + maxHistHeight
    ...
    with canvas(device, dither=True) as draw:
        ...
        # Histogram Outline
        draw.rectangle(
            (minHistLenght, maxHistHeight, maxHistLenght, minHistHeight),
            outline="white",
        )
        ...
        if ramBarWidth < maxRamBarW:
            draw.rectangle(
                (minRamBarW, minRamBarH, ramBarWidth, maxRamBarH), fill="white"
            )
            if ramUsd < 100:
        ...
        for htime in range(0, len(histogramTime) - 1):
            ...
            if histogramData[0] > maxHistHeight:
                draw.line(
                    (
                        histogramTime[timePlusOne],
                        histogramData[timePlusOne],
                        histogramTime[htime],
                        histogramData[htime],
                    ),
                    fill="orange",
                ...
                draw.text(
                    (
                        (minHistLenght + maxHistLenght) / 2,
                        (maxHistHeight + minHistHeight) / 2,
                    ),
                    "WARNING!",
                    fill="white",
                )
        ...
        draw.rectangle(
            (minHistLenght, maxHistHeight, minHistLenght + 27, maxHistHeight + 13),
            fill="black",
            outline="white",
        )
        draw.text(
            (minHistLenght + 2, maxHistHeight + 2),
            "{0:.2f}".format(cpuLoad[0]),
            fill="white",
        )
        ...
        if height > minBarHeight:
            ...
            if blnk == 1:
                ...
                draw.rectangle(
                    (110, minBarHeight, 124, minBarHeight + 10), fill="white"
                )
                draw.text((112, minBarHeight), str(tmpCel), fill="black")
                blnk = 0
            else:
                draw.rectangle(
                    (110, minBarHeight, 124, minBarHeight + 10),
                    fill="black",
                    outline="white",
                )
